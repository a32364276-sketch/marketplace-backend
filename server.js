...
app.post('/admin/add-deal', async (req, res) => {
  const { title, description, price, commission_percentage, merchant_id, image_url, inventory_limit, deal_expiry_date } = req.body;
...
if (commission < 0 || commission > 100) {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO deals (title, description, price, commission_percentage, merchant_id, image_url, inventory_limit, deal_expiry_date)
VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
 RETURNING *`,
     [title, description || '', price, commission, merchant_id, image_url || '', inventory_limit, deal_expiry_date]
    );
